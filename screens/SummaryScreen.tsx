...
export default function SummaryScreen({
  ...
  const convertedDate = (raw_date)=>{
    ...
    let convert_date = new Date(Date.UTC(split_date[0], split_date[1]-1, split_date[2], split_date[3], split_date[4], split_date[5])).toDateString();

    return moment(convert_date).format('MMMM DD YYYY')
  }
  
  // filter transactions by date
  const transactionFilterByDate = transactions.filter((item,index)=> 
          transactions.findIndex(obj =>
             obj.transac_by_fullname === item.transac_by_fullname && convertedDate(obj.transac_date) === convertedDate(item.transac_date)
            ) === index);
    ...
    <View style={styles.farmer_header} > 
       ...
       {  transactionFilterByDate.map((item)=>(
            ...
            <List.Accordion 
              ...
              left={props => <List.Icon {...props} icon="history" color={Colors.base} />}
              
            >
              
              {transactions.map((value)=>
               convertedDate(value.transac_date)  ==  convertedDate(item.transac_date) ?
               (   
                <List.Item  title={value.item_name+"("+value.quantity+")"} 
                            titleStyle={{fontFamily:'calibri-light'}}
                            description={"₱"+value.amount+' per '+value.unit_measure}
                            right={()=><Text style={{top:10}}>{"₱"+value.total_amount}</Text>} 
                />            
                ) : 
                null
              )} 
